...
class CLennart implements ISingleton {
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= LENNART_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = LENNART_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $le->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $le
    ...
    $le = &$this;
    // First the default Lennart themes/functions.php
  ...
  /**
   * Draw HTML for a menu defined in $le->config['menus'].
   *
