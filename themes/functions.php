...
<a href="http://validator.w3.org/checklink?uri={$le->request->current_url}">links</a>
<a href="http://qa-dev.w3.org/i18n-checker/index?async=false&amp;docAddr={$le->request->current_url}">i18n</a>
...
function get_debug() {
  ...
  if(isset($le->config['debug']['lennart']) && $le->config['debug']['lennart']) {
...
function render_views($region='default') {
  return CLennart::Instance()->views->Render($region);
}
...
function region_has_content($region='default' /*...*/) {
  return CLennart::Instance()->views->RegionHasView(func_get_args());
}
